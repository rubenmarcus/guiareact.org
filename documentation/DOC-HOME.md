...
Context api é uma forma de não praticarmos prop drilling,  que é basicamente uma situação em que os mesmos dados estão sendo enviados em quase todos os níveis devido a requisitos no nível final, exemplo, imagine a seguinte situação, você tem um elemento pai, que possui um elemento filho e dentro do filho outro elemento, e mais outro elemento e outro elemento, porém o dado principal esta lá no elemento pai, e você precisa exibi-lo no ultimo elemento declaro, já imaginou passar as props elementos por elementos a bagunça e nó que da na cabeça?

E AGORA QUEM PODERÁ NOS AJUDAR???

EUU!!! A CONTEXT API!!!!

Mas afinal o que é a context api? Bora analisar uma simpels declaração direto do reactjs que diz:
...
Legal, então traduzindo a Context API, fornece uma maneira de passar os dados de componentes sem ter que passar manualmente em todos os níveis.

Bora para um exemplo:

Vamos criar um arquivo chamado "CountContext.js", esse será o arquivo que gerenciará o nosso contexto:
...
beleza, mas o que isso faz? 

Em `const CountContext = createContext([0, () => {}]);` estamos criando um contexto que chamamos de `CountContext` e utilizamos a função `createContext` que recebe dois parametros um valor inicial de `0` e uma função, com isso criamos o nosso contexto e logo abaixo exportamos o mesmo.

Agora vamos em nosso arquivo App.jsx adicionar nossa context e dentro dela vamos adicionar os componentes que irão compartilhar as informações desse contexto:
...
```
O que fizemos acima foi usar nossa CountContext como qualquer outro componente, porém utilizando o `.Provider` para que assim possamos inserir componentes que poderão utilizar desse contexto. Com isso todos os componentes que estiverem dentro da tag `<CountContext.Provider>`poderão utilizar as funções de dentro do CountContext.

Vamos criar agora nosso componente counter.js
...
Note que estamos usando o hook do react chamado `useContext`, assim usaremos ele para receber as informações / funções de dentro do nosso contexto, e em nosso button incrementaremos o valor inicial do estado + 1.

Então sempre que clicarmos em nosso botão ele ira setar um novo estado adicionando e capturando diretamento do nosso contexto.

Resumindo: O Context é uma ferramenta poderosa dentro do React. Ela é usada para gerenciar os estados dos seus componentes sem a necessidade de um pacote para isso.
